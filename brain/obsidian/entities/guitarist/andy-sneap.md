---
id: andy-sneap
label: Andy Sneap
type: guitarist
zone: hard-rock-metal
roles: [guitarist, artist]
aliases: []
eraStart: 
eraPeak: 
primaryGenres: [hard rock]
curatorTags: [guitarist, hard rock, metal]
secondaryZones: []
layoutPinned: false
layoutX: 592
layoutY: -516
starter: false
---
# Andy Sneap

## Summary

Andy Sneap (born 18 July 1969) is an English guitarist, record producer, and composer with over 100 albums to his name, most of which have been produced at his Backstage Recording studios in rural Derbyshire. Some of the most critically acclaimed mixing-work on albums include Deliverance by Opeth and Disarm the Descent by Killswitch Engage; for which he earned a Swedish Grammis and a US Grammy nomination,...

## Connections

- [[accept|Accept]] - Wikipedia link signal (Wikipedia link: Accept (band))
- [[annihilator|Annihilator]] - Wikipedia link signal (Wikipedia link: Annihilator (band))
- [[anthrax-american-band|Anthrax (American band)]] - Wikipedia link signal (Wikipedia link: Anthrax (American band))
- [[artillery|Artillery]] - Wikipedia link signal (Wikipedia link: Artillery (band))
- [[as-i-lay-dying|As I Lay Dying]] - Wikipedia link signal (Wikipedia link: As I Lay Dying (band))
- [[caliban|Caliban]] - Wikipedia link signal (Wikipedia link: Caliban (band))
- [[carcass|Carcass]] - Wikipedia link signal (Wikipedia link: Carcass (band))

## Sources

- [Wikipedia](https://en.wikipedia.org/wiki/Andy_Sneap)
- [Wikidata](https://www.wikidata.org/wiki/Q527726)

## Curator Notes

Use this section for human notes. Machine-readable organization belongs in the frontmatter above.
