---
id: glam-rock
label: Glam rock
type: genre
zone: rock-circuit
roles: [genre]
aliases: []
eraStart: 
eraPeak: 
primaryGenres: [Glam rock]
curatorTags: [rock, glam rock, hard rock]
secondaryZones: []
layoutPinned: false
layoutX: -7
layoutY: -302
starter: false
---
# Glam rock

## Summary

A theatrical rock branch where big hooks, spectacle and electric guitar identity became tightly connected.

## Connections

- [[brian-may-red-special|Brian May Red Special]] - layered Queen tone
- [[hard-rock|Hard rock]] - stage volume overlap
- [[queen|Queen]] - theatrical rock
- [[rock|Rock]] - theatrical branch

## Sources

- [Wikipedia](https://en.wikipedia.org/wiki/Glam_rock)

## Curator Notes

Use this section for human notes. Machine-readable organization belongs in the frontmatter above.
