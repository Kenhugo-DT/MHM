---
id: power-metal
label: power metal
type: genre
zone: hard-rock-metal
roles: [genre]
aliases: []
eraStart: 
eraPeak: 
primaryGenres: [power metal]
curatorTags: [heavy metal, speed metal, hard rock]
secondaryZones: []
layoutPinned: false
layoutX: 1765
layoutY: 227
starter: false
---
# power metal

## Summary

Power metal is a subgenre of heavy metal music combining characteristics of traditional heavy metal with speed metal, often within a symphonic context. Generally, power metal is characterized by a faster, lighter, and more uplifting sound, in contrast with the heaviness and dissonance prevalent in, for example, extreme metal.

## Connections

- [[accept|Accept]] - Wikipedia link signal (Wikipedia link: Accept (band))
- [[angra|Angra]] - Wikipedia link signal (Wikipedia link: Angra (band))
- [[avian|Avian]] - Wikipedia link signal (Wikipedia link: Avian (band))

## Sources

- [Wikipedia](https://en.wikipedia.org/wiki/Power_metal)
- [Wikidata](https://www.wikidata.org/wiki/Q57143)

## Curator Notes

Use this section for human notes. Machine-readable organization belongs in the frontmatter above.
